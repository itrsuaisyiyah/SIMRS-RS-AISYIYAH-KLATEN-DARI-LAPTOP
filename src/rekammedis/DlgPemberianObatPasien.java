...
public class DlgPemberianObatPasien extends javax.swing.JDialog {
    ...
    private String kdobat = "", status = "", statusOK = "", dataDipilih = "", kdobatFix = "", waktuSimpan = "",
        ...
        Object[] row = {"Cek", "No. Rawat", "No. RM", "Nama Pasien", "","Rg. Rawat/Inst./Poli", "Nama Obat", "Jns. Obat", 
            "Jumlah", "Dosis", "Cara Pemberian/Rute","Petugas Double Check 1","Petugas Double Check 2", "Tgl. Beri", 
            "Jam 1", "Ket. 1", "Jam 2", "Ket. 2", "Jam 3", "Ket. 3", "Jam 4", "Ket. 4", 
            "Jam 5", "Ket. 5", "Jam 6", "Ket. 6", "Jam 7", "Ket. 7", "Jam 8", "Ket. 8",
            "Jlh. Sisa Obat", "Jlh. Sisa Obat","Jlh. Sisa Obat","kode_brng", "tgl_pemberian",
...
tbObat.setDefaultRenderer(Boolean.class, tbObat.getDefaultRenderer(Boolean.class));
    ...
    private void initComponents() {
        ...
        TtglBeri.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "06-07-2026" }));
        ...
        Tket2.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                Tket2ActionPerformed(evt);
            }
        });
    ...
    private void BtnHapusActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnHapusActionPerformed
       if (tbObat.getSelectedRow() > -1) {
        x = JOptionPane.showConfirmDialog(rootPane,
        ...
        if (x == JOptionPane.YES_OPTION) {
            ...
            // Hitung data yang dicentang
            x = 0;
            for (i = 0; i < tbObat.getRowCount(); i++) {
                if (Boolean.TRUE.equals(tbObat.getValueAt(i, 0))) {
                    x++;
                }
            }
            ...
            if (x == 0) {
                ...
                Object waktu = tbObat.getValueAt(tbObat.getSelectedRow(), 49);

                if (waktu == null || waktu.toString().trim().equals("")) {
                    JOptionPane.showMessageDialog(rootPane,
                            "Pilih baris DETAIL OBAT, bukan baris pasien.");
                    return;
                }

                if (Sequel.queryu2tf(
                        "delete from pemberian_obat where waktu_simpan=?",
                        1,
                        new String[]{waktu.toString()})) {
                ...
                // ===========================
                // Hapus berdasarkan conteng
                // ===========================
                for (i = 0; i < tbObat.getRowCount(); i++) {

                    if (Boolean.TRUE.equals(tbObat.getValueAt(i, 0))) {

                        Object waktu = tbObat.getValueAt(i, 49);
        ...
        JOptionPane.showMessageDialog(rootPane,
                "Silahkan pilih salah satu datanya terlebih dahulu..!!");
    }
            ...
            if (tbObat.getSelectedRow() > -1) {
                ...
                Sequel.mengedit("pemberian_obat", "waktu_simpan=?", "nama_obat=?, dosis=?, cara_pemberian=?, "
                        ...
                        + "cek_dobel=?, nip_petugas1=?, nip_petugas2=?, sift=?", 38, new String[]{
                            ...
                            tbObat.getValueAt(tbObat.getSelectedRow(), 49).toString()
                        });
                tampil();
                emptTeks();
            } else {
    ...
    private void Tket2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_Tket2ActionPerformed
        // TODO add your handling code here:
    }//GEN-LAST:event_Tket2ActionPerformed
    ...
    */
    ...
    private void tampil() {
        ...
        kunciCacheResep = null; // reset cache resep supaya klik baris berikutnya selalu ambil data terbaru
        // =====================================================================
        // OPTIMASI: query lama menjalankan 1 query header (daftar pasien)
        // lalu, DI DALAM LOOP, menjalankan 1 query detail per pasien (N+1 query).
        // Kalau ada 200 pasien pada rentang tanggal terpilih, itu artinya 201
        // roundtrip ke database. Versi ini hanya menjalankan 2 query TOTAL,
        // berapa pun jumlah pasiennya, lalu detail dikelompokkan di memori
        // (Java) berdasarkan no_rawat. Struktur & urutan kolom tabel TIDAK
        // berubah sama sekali, jadi seluruh fungsi lain (simpan, hapus, cetak,
        // cek dobel, dsb.) tetap bekerja seperti sebelumnya.
        // =====================================================================
        try {
            boolean semuaJenis = (cmbJnsObat.getSelectedIndex() == 0);
            String tgl1 = Valid.SetTgl(DTPCari1.getSelectedItem() + "");
            String tgl2 = Valid.SetTgl(DTPCari2.getSelectedItem() + "");
            String cariTeks = "%" + TCari.getText().trim() + "%";
            String jnsObatTerpilih = semuaJenis ? null : cmbJnsObat.getSelectedItem().toString();

            // -----------------------------------------------------------
            // 1) Query header: daftar pasien (no_rawat) yang cocok filter
            // -----------------------------------------------------------
            if (semuaJenis) {
                ...
                ps.setString(1, tgl1);
                ps.setString(2, tgl2);
                ps.setString(3, cariTeks);
                ps.setString(4, tgl1);
                ps.setString(5, tgl2);
                ps.setString(6, cariTeks);
                ps.setString(7, tgl1);
                ps.setString(8, tgl2);
                ps.setString(9, cariTeks);
            } else {
                ps = koneksi.prepareStatement("SELECT po.no_rawat, p.no_rkm_medis, p.nm_pasien from pemberian_obat po "
                        + "inner join reg_periksa rp on rp.no_rawat=po.no_rawat inner join pasien p on p.no_rkm_medis=rp.no_rkm_medis where "
                ...
                ps.setString(1, tgl1);
                ps.setString(2, tgl2);
                ps.setString(3, "%" + jnsObatTerpilih + "%");
                ps.setString(4, cariTeks);
                ps.setString(5, tgl1);
                ps.setString(6, tgl2);
                ps.setString(7, "%" + jnsObatTerpilih + "%");
                ps.setString(8, cariTeks);
                ps.setString(9, tgl1);
                ps.setString(10, tgl2);
                ps.setString(11, "%" + jnsObatTerpilih + "%");
                ps.setString(12, cariTeks);
            }

            // -----------------------------------------------------------
            // 2) Query detail: SEMUA baris pemberian_obat pada rentang
            //    tanggal (dan jenis obat bila difilter) dalam SATU query,
            //    lalu dikelompokkan per no_rawat di memori.
            // -----------------------------------------------------------
            java.util.Map<String, java.util.List<Object[]>> detailPerPasien = new java.util.LinkedHashMap<>();
            try {
                if (semuaJenis) {
                    ps2 = koneksi.prepareStatement(
                        "SELECT pemberian_obat.*, " +
                        "pg1.nama AS petugas1, " +
                        "pg2.nama AS petugas2, " +
                        "nm_unit, nama_obat, jenis_obat, jlh_obat, dosis, cara_pemberian, " +
                        "date_format(tgl_pemberian,'%d-%m-%Y') tgl_beri, " +
                        ...
                        "if(cek_jam8='ya',time_format(jadwal_pemberian8,'%H:%i'),'') jam8, " +
                        "jlh_sisa_obat, kode_brng, tgl_pemberian, " +
                        "cek_jam1, cek_jam2, cek_jam3, cek_jam4, cek_jam5, cek_jam6, cek_jam7, cek_jam8, " +
                        "jadwal_pemberian, jadwal_pemberian2, jadwal_pemberian3, jadwal_pemberian4, " +
                        "jadwal_pemberian5, jadwal_pemberian6, jadwal_pemberian7, jadwal_pemberian8, waktu_simpan " +
                        "FROM pemberian_obat " +
                        "LEFT JOIN pegawai pg1 ON pg1.nik = pemberian_obat.nip_petugas1 " +
                        "LEFT JOIN pegawai pg2 ON pg2.nik = pemberian_obat.nip_petugas2 " +
                        "WHERE tgl_pemberian BETWEEN ? AND ? " +
                        "ORDER BY no_rawat, status, tgl_pemberian, waktu_simpan"
                    );
                    ps2.setString(1, tgl1);
                    ps2.setString(2, tgl2);
                } else {
                    ps2 = koneksi.prepareStatement("SELECT pemberian_obat.*,\n" +
                            "       pg1.nama AS petugas1,\n" +
                            "       pg2.nama AS petugas2, nm_unit, nama_obat, jenis_obat, jlh_obat, dosis, cara_pemberian, date_format(tgl_pemberian,'%d-%m-%Y') tgl_beri, "
                            + "if(cek_jam1='ya',time_format(jadwal_pemberian,'%H:%i'),'') jam1, "
                            + "if(cek_jam2='ya',time_format(jadwal_pemberian2,'%H:%i'),'') jam2, "
                            + "if(cek_jam3='ya',time_format(jadwal_pemberian3,'%H:%i'),'') jam3, "
                            + "if(cek_jam4='ya',time_format(jadwal_pemberian4,'%H:%i'),'') jam4, "
                            ...
                            + "jadwal_pemberian7, jadwal_pemberian8, waktu_simpan FROM pemberian_obat\n" +
                            "LEFT JOIN petugas pg1 ON pg1.nip = pemberian_obat.nip_petugas1\n" +
                            "LEFT JOIN petugas pg2 ON pg2.nip = pemberian_obat.nip_petugas2\n" +
                            "WHERE "
                            + "tgl_pemberian between ? and ? "
                            + "and jenis_obat=? "
                            + "order by no_rawat, status, tgl_pemberian, waktu_simpan");
                    ps2.setString(1, tgl1);
                    ps2.setString(2, tgl2);
                    ps2.setString(3, jnsObatTerpilih);
                }

                rs2 = ps2.executeQuery();
                while (rs2.next()) {
                    String noRawatBaris = rs2.getString("no_rawat");
                    detailPerPasien.computeIfAbsent(noRawatBaris, k -> new java.util.ArrayList<>()).add(new Object[]{
                        false, "", "", "", "",
                        ...
                        rs2.getString("petugas1"),
                        rs2.getString("petugas2"),
            ...
            // -----------------------------------------------------------
            // 3) Susun tabel: 1 baris header pasien + baris-baris detail
            //    yang sudah tersedia di memori (tanpa query tambahan).
            // -----------------------------------------------------------
            try {
                rs = ps.executeQuery();
                while (rs.next()) {
                    ...
                    java.util.List<Object[]> detailList = detailPerPasien.get(noRawat);
                    if (detailList != null) {
                        for (Object[] baris : detailList) {
    ...
    private void getData() {
        ...
        if (tbObat.getSelectedRow() != -1) {
            waktuSimpan = tbObat.getValueAt(tbObat.getSelectedRow(), 49).toString();
    ...
    // ==========================================================================
    // CACHE DAFTAR OBAT (databarang)
    // ==========================================================================
    // Daftar obat (databarang) adalah data MASTER yang jarang berubah (hanya
    // saat farmasi menambah/mengubah item obat), BUKAN data transaksi pasien.
    // Jendela "Cari Obat" bisa dibuka & dicari berkali-kali dalam satu shift,
    // dan sebelumnya SETIAP pencarian selalu query ke database.
    //
    // - cacheDaftarObat / cacheDaftarObatWaktu (static): cache di MEMORI,
    //   dipakai selama masih segar (CACHE_OBAT_TTL_MS). Ini yang benar-benar
    //   mempercepat, karena tidak perlu roundtrip ke DB sama sekali selama
    //   cache masih segar - pencarian teks dilakukan di Java (in-memory).
    // - CACHE_OBAT_FILE: file cadangan di disk, HANYA dipakai sebagai fallback
    //   kalau koneksi ke database sedang bermasalah saat cache memori kosong
    //   (misal: aplikasi baru dibuka & jaringan/DB lagi gangguan). ISI FILE
    //   INI SELALU DITIMPA dengan data terbaru setiap kali berhasil mengambil
    //   dari DB, jadi tidak akan pernah dipakai kalau DB normal.
    //
    // PENTING: pola cache seperti ini SENGAJA hanya diterapkan untuk daftar
    // obat (data master), TIDAK untuk data pemberian obat / resep pasien.
    // Data pemberian obat & resep adalah data transaksi yang berubah tiap
    // saat dan dipakai banyak user/komputer sekaligus, jadi kalau disimpan
    // di file lokal, resikonya user bisa melihat status yang sudah usang
    // (misalnya info double-check atau jadwal minum obat yang sudah basi) -
    // ini berbahaya untuk keselamatan pasien. Untuk data semacam itu, cache
    // yang aman hanya cache memori jangka pendek yang sudah diterapkan
    // sebelumnya (tampilResepJikaPerlu()), bukan cache file.
    // ==========================================================================
    private static java.util.List<Object[]> cacheDaftarObat = null;
    private static long cacheDaftarObatWaktu = 0L;
    private static final long CACHE_OBAT_TTL_MS = 5 * 60 * 1000; // segar selama 5 menit
    private static final String CACHE_OBAT_FILE =
            System.getProperty("java.io.tmpdir") + java.io.File.separator + "simrs_cache_databarang.txt";

    private void tampilObat() {
        Valid.tabelKosong(tabMode1);
        String kataKunci = TCari1.getText().trim().toLowerCase();
        for (Object[] baris : ambilDaftarObatDenganCache()) {
            String kodeBrng = String.valueOf(baris[0]).toLowerCase();
            String namaBrng = String.valueOf(baris[1]).toLowerCase();
            String kodeSat = String.valueOf(baris[2]).toLowerCase();
            if (kataKunci.isEmpty()
                    || kodeBrng.contains(kataKunci)
                    || namaBrng.contains(kataKunci)
                    || kodeSat.contains(kataKunci)) {
                tabMode1.addRow(baris);
            }
        }
    }

    private java.util.List<Object[]> ambilDaftarObatDenganCache() {
        long sekarang = System.currentTimeMillis();
        boolean cacheMemoriMasihSegar = cacheDaftarObat != null
                && (sekarang - cacheDaftarObatWaktu) < CACHE_OBAT_TTL_MS;
        if (cacheMemoriMasihSegar) {
            return cacheDaftarObat;
        }
        try {
            java.util.List<Object[]> dariDB = ambilDaftarObatDariDB();
            cacheDaftarObat = dariDB;
            cacheDaftarObatWaktu = sekarang;
            tulisCacheObatKeFile(dariDB);
            return dariDB;
        } catch (Exception e) {
            System.out.println("Notifikasi : gagal ambil daftar obat dari DB, coba cache. " + e);
            if (cacheDaftarObat != null) {
                return cacheDaftarObat; // masih ada cache lama di memori, dari sesi ini
            }
            java.util.List<Object[]> dariFile = bacaCacheObatDariFile();
            if (dariFile != null) {
                cacheDaftarObat = dariFile;
                cacheDaftarObatWaktu = sekarang;
                return dariFile;
            }
    ...
    private java.util.List<Object[]> ambilDaftarObatDariDB() throws SQLException {
        java.util.List<Object[]> hasil = new java.util.ArrayList<>();
        PreparedStatement psObat = koneksi.prepareStatement(
                "SELECT kode_brng, nama_brng, kode_sat FROM databarang ORDER BY nama_brng");
        try {
    ...
    private void tulisCacheObatKeFile(java.util.List<Object[]> daftar) {
        try {
            java.io.BufferedWriter bw = new java.io.BufferedWriter(new java.io.OutputStreamWriter(
                    new java.io.FileOutputStream(CACHE_OBAT_FILE), "UTF-8"));
            try {
                for (Object[] baris : daftar) {
                    bw.write(String.valueOf(baris[0]) + "\u0001" + String.valueOf(baris[1]) + "\u0001" + String.valueOf(baris[2]));
                    bw.newLine();
                }
            } finally {
                bw.close();
            ...
            System.out.println("Notifikasi : gagal menulis cache obat ke file. " + e);
        }
    }

    private java.util.List<Object[]> bacaCacheObatDariFile() {
        java.io.File berkas = new java.io.File(CACHE_OBAT_FILE);
        if (!berkas.exists()) {
            return null;
        }
        java.util.List<Object[]> hasil = new java.util.ArrayList<>();
        try {
            java.io.BufferedReader br = new java.io.BufferedReader(new java.io.InputStreamReader(
                    new java.io.FileInputStream(berkas), "UTF-8"));
            try {
                String baris;
                while ((baris = br.readLine()) != null) {
                    String[] kolom = baris.split("\u0001", -1);
                    if (kolom.length == 3) {
                        hasil.add(new Object[]{kolom[0], kolom[1], kolom[2]});
                    }
                }
            } finally {
                br.close();
            ...
            System.out.println("Notifikasi : gagal membaca cache obat dari file. " + e);
            return null;
        }
    ...
    private void tampilData() {
        try {
            ...
            "SELECT po.*, p.no_rkm_medis, p.nm_pasien, " +
            "pt1.nama AS ptgs1, " +
            "pt2.nama AS ptgs2 " +
            "FROM pemberian_obat po " +
            "INNER JOIN reg_periksa rp ON rp.no_rawat=po.no_rawat " +
            "INNER JOIN pasien p ON p.no_rkm_medis=rp.no_rkm_medis " +
            "LEFT JOIN petugas pt1 ON pt1.nip=po.nip_petugas1 " +
            "LEFT JOIN petugas pt2 ON pt2.nip=po.nip_petugas2 " +
            "WHERE po.waktu_simpan=?"
        );
            try {
                ps3.setString(1, waktuSimpan);
    ...
    // ==========================================================================
    // OPTIMASI: dipanggil setiap kali user klik/pindah baris DETAIL obat pada
    // tabel (tampilData()). Satu pasien pada satu tanggal biasanya punya banyak
    // baris detail (jam1..jam8, beberapa obat, dst), padahal daftar resepnya
    // SAMA untuk semua baris itu. Tanpa cache ini, tiap klik baris menjalankan
    // ulang query join 5 tabel (resep_dokter, resep_obat, reg_periksa, pasien,
    // dokter, databarang) walau hasilnya persis sama dengan klik sebelumnya.
    // Query hanya dijalankan ulang kalau pasien / tanggal / jenis resep (ranap
    // atau ralan) benar-benar berubah. Tombol "Cari Resep" (BtnResepActionPerformed)
    // dan setData() tetap memanggil tampilResep() langsung (tanpa cache) supaya
    // selalu mengambil data terbaru saat diminta eksplisit oleh user.
    // ==========================================================================
    private void tampilResepJikaPerlu() {
        String kunciSekarang = cmbResep.getSelectedIndex() + "|" + TNoRW.getText() + "|"
                + Valid.SetTgl(tgl_beri.getSelectedItem() + "");
        if (!kunciSekarang.equals(kunciCacheResep)) {
            tampilResep();
            kunciCacheResep = kunciSekarang;
        }
    }
        ...
        try {
            ...
            if (cmbResep.getSelectedIndex() == 1) {                
              ps4=koneksi.prepareStatement("select resep_dokter.no_resep,resep_dokter.kode_brng,resep_dokter.jml,resep_dokter.aturan_pakai,resep_obat.tgl_peresepan,resep_obat.jam_peresepan,"+
                    ...
                    " resep_obat.tgl_peresepan=? and resep_obat.no_rawat=? order by resep_obat.tgl_perawatan,resep_obat.jam desc");
                //resep rawat inap
            } else if (cmbResep.getSelectedIndex() == 0) {                
               ps4=koneksi.prepareStatement("select resep_dokter.no_resep,resep_dokter.kode_brng,resep_dokter.jml,resep_dokter.aturan_pakai,resep_obat.tgl_peresepan,resep_obat.jam_peresepan,"+
                    " resep_obat.no_rawat,pasien.no_rkm_medis,pasien.nm_pasien,resep_obat.kd_dokter,dokter.nm_dokter, "+
            ...
            try {
                ps4.setString(1, Valid.SetTgl(tgl_beri.getSelectedItem() + ""));
                ps4.setString(2, TNoRW.getText());
