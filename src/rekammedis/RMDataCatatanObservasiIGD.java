...
public final class RMDataCatatanObservasiIGD extends javax.swing.JDialog {
    ...
    public RMDataCatatanObservasiIGD(java.awt.Frame parent, boolean modal) {
        ...
        tabMode=new DefaultTableModel(null,new Object[]{
            ...
            "TD(mmHg)","HR(x/menit)","RR(x/menit)","Suhu(°C)","SpO2(%)","Tindakan","NIP","Nama Petugas"
        }){
    ...
    private void initComponents() {
        ...
        Tanggal.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "06-10-2023" }));
        ...
        jLabel8.setBounds(830, 10, 120, 23);
        ...
        scrollPane6.setViewportView(tindakan);

        FormInput.add(scrollPane6);
        scrollPane6.setBounds(830, 30, 290, 60);
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        if(TNoRw.getText().trim().equals("")||TPasien.getText().trim().equals("")){
            ...
            if(Sequel.menyimpantf("catatan_observasi_igd","?,?,?,?,?,?,?,?,?,?,?","Data",11,new String[]{
                ...
                GCS.getText(),TD.getText(),HR.getText(),RR.getText(),Suhu.getText(),SPO.getText(),NIP.getText(),tindakan.getText()
            })==true){
                tampil();
                emptTeks();
            }   
    ...
    */
            ...
            try {
                ...
                while(rs.next()){
                    tabMode.addRow(new String[]{
                        ...
                        rs.getString("hr"),rs.getString("rr"),rs.getString("suhu"),rs.getString("spo2"),rs.getString("tindakan"),rs.getString("nip"),
                        rs.getString("nama")
                    });
                }
            } catch (Exception e) {
    ...
    private void ganti() {
       /*   if(tbObat.getSelectedRow()!= -1){
                if(Sequel.mengedittf("rujuk","no_rujuk=?","no_rawat=?,tgl_rujuk=?,rujuk_ke=?,bagian=?,kat_rujuk=?,ambulance=?,keterangan=?,nokendaraan=?,alasanrujuk=?",pendamping=?",namapendamping=?",ku=?",gcs=?",respirasi=?",suhu=?",tensi=?",nadi=?",spo2=?",alergi=?",pemeriksaan=?",pemeriksaanpj=?",pemeriksaanlab=?",terapi=?",lainnya=?",tindakan=?",keterangan_diagnosa=?",kd_dokter=?",jam=?", 11,new String[]{
            ...
            */ 
        Sequel.mengedit("catatan_observasi_igd","tgl_perawatan=? and jam_rawat=? and no_rawat=?","no_rawat=?,tgl_perawatan=?,jam_rawat=?,gcs=?,td=?,hr=?,rr=?,suhu=?,spo2=?,nip=?,tindakan=?",14,new String[]{
            TNoRw.getText(),Valid.SetTgl(Tanggal.getSelectedItem()+""),Jam.getSelectedItem()+":"+Menit.getSelectedItem()+":"+Detik.getSelectedItem(),
            GCS.getText(),TD.getText(),HR.getText(),RR.getText(),Suhu.getText(),SPO.getText(),tindakan.getText(),NIP.getText(),tbObat.getValueAt(tbObat.getSelectedRow(),6).toString(),
