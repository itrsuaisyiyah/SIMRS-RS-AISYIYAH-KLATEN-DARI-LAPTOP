...
public final class RMHasilPemeriksaanEchoPediatrik extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        Tanggal.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "08-04-2026 14:53:07" }));
    ...
    private void btnUploudActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnUploudActionPerformed
        if(tabModeDicom.getRowCount()==0){
            ...
            if(tbListDicom.getSelectedRow()!= -1){
                this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
                Sequel.queryu2("delete from hasil_pemeriksaan_echo_pediatrik_gambar where no_rawat=?",1,new String[]{tbObat.getValueAt(tbObat.getSelectedRow(),0).toString()});
                ApiOrthanc orthanc=new ApiOrthanc();
                orthanc.AmbilJpg(tbListDicom.getValueAt(tbListDicom.getSelectedRow(),2).toString());
                try {
                    CloseableHttpClient httpClient = HttpClients.createDefault();
                    HttpPost post = new HttpPost("http://"+koneksiDB.HOSTHYBRIDWEB()+":"+koneksiDB.PORTWEB()+"/"+koneksiDB.HYBRIDWEB()+"/hasilpemeriksaanechopediatrik/pages/upload/service.php");
                    System.out.println("http://"+koneksiDB.HOSTHYBRIDWEB()+":"+koneksiDB.PORTWEB()+"/"+koneksiDB.HYBRIDWEB()+"/hasilpemeriksaanechopediatrik/pages/upload/service.php");
