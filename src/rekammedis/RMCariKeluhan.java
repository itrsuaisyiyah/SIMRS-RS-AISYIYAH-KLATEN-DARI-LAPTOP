...
public final class RMCariKeluhan extends javax.swing.JDialog {
    ...
    public RMCariKeluhan(java.awt.Frame parent, boolean modal) {
        ...
        Object[] row={"Tanggal","Keluhan Utama"};
    ...
    */
    ...
    // Daftar tabel penilaian medis yang menjadi sumber data keluhan utama.
    // Data akan menyesuaikan otomatis: tabel mana saja yang punya data
    // untuk no_rawat yang bersangkutan, itu yang akan tampil.
    private static final String[] TABEL_SUMBER = {
    ...
    public void tampil() {
        ...
        try{
            ...
            for (int i = 0; i < TABEL_SUMBER.length; i++) {
                String t = TABEL_SUMBER[i];
                if (i > 0) {
                    sql.append(" union all ");
                }
                sql.append("select ").append(t).append(".tanggal,")
                   .append(t).append(".keluhan_utama from ").append(t)
                   .append(" where ").append(t).append(".no_rawat=? and (")
                   .append(t).append(".tanggal like ? or ")
                   .append(t).append(".keluhan_utama like ?)");
            }
            sql.append(" order by tanggal");

            ps=koneksi.prepareStatement(sql.toString());
            try{
                int idx=1;
                for (int i = 0; i < TABEL_SUMBER.length; i++) {
                    ps.setString(idx++,norawat);
