                ...
                if(NoPermintaan.trim().equals("")||DiagnosaKlinis.trim().equals("")){
                    ...
                    try {
                        ...
                        ps=koneksi.prepareStatement(
                            "select permintaan_labmb.noorder,permintaan_labmb.no_rawat,reg_periksa.no_rkm_medis,pasien.nm_pasien,permintaan_labmb.tgl_permintaan,pasien.tgl_lahir,"+
                            "permintaan_labmb.jam_permintaan,pasien.alamat,kelurahan.nm_kel,kecamatan.nm_kec,kabupaten.nm_kab,propinsi.nm_prop,"+
                            "if(pasien.jk='L','1','2') as jk,permintaan_labmb.dokter_perujuk,dokter.nm_dokter,reg_periksa.kd_poli,"+
                            "poliklinik.nm_poli,permintaan_labmb.informasi_tambahan,permintaan_labmb.diagnosa_klinis "+
                            ...
                            "where permintaan_labmb.noorder=?");
                        try {
                            ...
                            if(rs.next()){
                                ...
                                Permintaan="";
                                ps2=koneksi.prepareStatement("select permintaan_pemeriksaan_labmb.kd_jenis_prw from permintaan_pemeriksaan_labmb where permintaan_pemeriksaan_labmb.noorder=?");
                                try {
                                    ps2.setString(1,NoPermintaan);
                                    rs2=ps2.executeQuery();
                                    while(rs2.next()){
                                ...
                                Permintaan="";
                                ps2=koneksi.prepareStatement("select permintaan_detail_permintaan_labmb.id_template from permintaan_detail_permintaan_labmb where permintaan_detail_permintaan_labmb.noorder=?");
                                try {
                                    ps2.setString(1,NoPermintaan);
                                    rs2=ps2.executeQuery();
                                    while(rs2.next()){
            ...
            if(!NoRawat.equals("")){
                ...
                if(NoPermintaan.trim().equals("")||DiagnosaKlinis.trim().equals("")){
                    ...
                    try {
                        ...
                        ps=koneksi.prepareStatement(
                            "select permintaan_labmb.noorder,permintaan_labmb.no_rawat,reg_periksa.no_rkm_medis,pasien.nm_pasien,permintaan_labmb.tgl_permintaan,pasien.tgl_lahir,"+
                            "permintaan_labmb.jam_permintaan,pasien.alamat,kelurahan.nm_kel,kecamatan.nm_kec,kabupaten.nm_kab,propinsi.nm_prop,if(pasien.jk='L','1','2') as jk,"+
                            "permintaan_labmb.dokter_perujuk,dokter.nm_dokter,kamar_inap.kd_kamar,bangsal.kd_bangsal,bangsal.nm_bangsal,permintaan_labmb.informasi_tambahan,permintaan_labmb.diagnosa_klinis "+
                            "from permintaan_labmb inner join reg_periksa on permintaan_labmb.no_rawat=reg_periksa.no_rawat "+
                            "inner join pasien on reg_periksa.no_rkm_medis=pasien.no_rkm_medis "+
                            "inner join dokter on permintaan_labmb.dokter_perujuk=dokter.kd_dokter "+
                            ...
                            "inner join kabupaten on pasien.kd_kab=kabupaten.kd_kab "+
                            "inner join propinsi on pasien.kd_prop=propinsi.kd_prop "+
                            "where permintaan_labmb.noorder=? group by permintaan_labmb.noorder");
                        try {
                            ps.setString(1,NoPermintaan);
                            rs=ps.executeQuery();
                            if(rs.next()){
