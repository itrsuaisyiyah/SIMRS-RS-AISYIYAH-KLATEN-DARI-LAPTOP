...
public class DlgTagihanOperasi extends javax.swing.JDialog {
    ...
    private final java.util.concurrent.atomic.AtomicBoolean sedangMuatTindakan = new java.util.concurrent.atomic.AtomicBoolean(false);
    private final java.util.concurrent.atomic.AtomicBoolean sedangMuatObat = new java.util.concurrent.atomic.AtomicBoolean(false);
    ...
    public DlgTagihanOperasi(java.awt.Frame parent, boolean modal) {
        ...
        // DIPINDAHKAN dari formWindowOpened() ke constructor.
        // formWindowOpened() hanya terpanggil SEKALI seumur hidup dialog (event windowOpened
        // di Swing/AWT tidak nyala lagi kalau dialog di-reuse untuk pasien berikutnya), sehingga
        // cara_bayar_operasi/kelas_operasi bisa masih bernilai default ("Yes"/"Yes") saat tampil()
        // otomatis dipanggil dari setNoRm() -> query jadi salah filter -> tabel Tindakan kosong,
        // dan baru benar setelah user klik tombol centang hijau (setelah window sempat terbuka).
        // Dipindah ke sini supaya SELALU dimuat sebelum tampil()/tampil2() dipanggil.
            ...
            TCariPaket.getDocument().addDocumentListener(new javax.swing.event.DocumentListener(){
                ...
                public void insertUpdate(DocumentEvent e) {
                    if(TCariPaket.getText().length()>2){
                        runBackground(() ->tampil(), sedangMuatTindakan);
                    }
                }
                @Override
            ...
            TCari.getDocument().addDocumentListener(new javax.swing.event.DocumentListener(){
                ...
                public void insertUpdate(DocumentEvent e) {
                    if(TCari.getText().length()>2){
                        runBackground(() ->tampil2(), sedangMuatObat);
                    }
                }
                @Override
                public void removeUpdate(DocumentEvent e) {
    ...
    private void initComponents() {
        ...
        Kategori.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "-", "Kecil", "Sedang", "Besar", "Khusus" }));
...
private void ChkInputActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ChkInputActionPerformed
   isForm();  
    ...
    private void formWindowOpened(java.awt.event.WindowEvent evt) {//GEN-FIRST:event_formWindowOpened
        // Isi method ini sudah dipindahkan ke constructor (lihat komentar di sana) supaya
        // selalu jalan sebelum tampil()/tampil2() dipanggil, tidak tergantung timing event
        // windowOpened yang cuma nyala sekali seumur hidup dialog.
    ...
    */
    ...
    private void tampil() {
        isUpdating = true; // Kunci proses di sini
    try {
        // ... (Logika pengisian tabel atau tabMode.setDataVector Anda) ...
    } finally {
        isUpdating = false; // Buka kunci setelah selesai
    }
        jml = 0;
    ...
    private void runBackground(Runnable task, java.util.concurrent.atomic.AtomicBoolean sedangMuat) {
        if (!sedangMuat.compareAndSet(false, true)) return;
        if (executor.isShutdown() || executor.isTerminated()) { sedangMuat.set(false); return; }

        if (isDisplayable()) {
            setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
        }

        // PENTING: task di sini menyentuh JTable/TableModel (tabMode/tabMode2) secara langsung.
        // Komponen Swing HANYA boleh diakses/diubah dari Event Dispatch Thread (EDT).
        // Menjalankannya di background thread (executor) menyebabkan race condition dengan
        // proses paint() milik JTable -> ArrayIndexOutOfBoundsException saat tabel di-refresh.
        // Karena tampil()/tampil2() mencampur query DB dan update tabel jadi satu blok,
        // task dijalankan di EDT lewat invokeLater agar aman (bukan lewat executor.submit).
        // Flag busy dipisah per task (tindakan vs obat) supaya panggilan runBackground(tampil)
        // dan runBackground(tampil2) yang terjadi berurutan tidak saling memblokir satu sama lain.
        SwingUtilities.invokeLater(() -> {
            try {
                task.run();
            } finally {
                sedangMuat.set(false);
                if (isDisplayable()) {
