...
public class DlgCariPeriksaRadiologi extends javax.swing.JDialog {
    ...
    public DlgCariPeriksaRadiologi(java.awt.Frame parent, boolean modal) {
        ...
        Object[] row={"No.Rawat","Pasien","Petugas","Tgl.Periksa","Jam Periksa","Dokter Perujuk","Penanggung Jawab","No WA Pasien"};
        tabMode=new DefaultTableModel(null,row){
              @Override public boolean isCellEditable(int rowIndex, int colIndex){return false;}
        };
        tbDokter.setModel(tabMode);

        tbDokter.setPreferredScrollableViewportSize(new Dimension(800,900));
    ...
    private void initComponents() {
        ...
        BtnKirimWA.setMnemonic('U');
        BtnKirimWA.setText("Kirim");
        BtnKirimWA.setToolTipText("Alt+U");
        ...
        BtnWAHasil.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/whatsapp.png"))); // NOI18N
        BtnWAHasil.setMnemonic('T');
        BtnWAHasil.setText("Kirim");
        BtnWAHasil.setToolTipText("Alt+T");
        BtnWAHasil.setName("BtnWAHasil"); // NOI18N
        BtnWAHasil.setPreferredSize(new java.awt.Dimension(100, 30));
        BtnWAHasil.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                BtnWAHasilActionPerformed(evt);
            }
        });
        BtnWAHasil.addKeyListener(new java.awt.event.KeyAdapter() {
            public void keyPressed(java.awt.event.KeyEvent evt) {
                BtnWAHasilKeyPressed(evt);
            }
        });
        panelGlass6.add(BtnWAHasil);
...
private void tbDokterKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_tbDokterKeyPressed
   if(tabMode.getRowCount()!=0){
    ...
    private void BtnKirimWAActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnKirimWAActionPerformed
        // TODO add your handling code here:
        String FileName = "RAD_" + tbDokter.getValueAt(tbDokter.getSelectedRow(), 0).toString().replaceAll("/", "");
//       String NoTlp = Sequel.cariIsi("SELECT p.no_tlp FROM pasien p JOIN reg_periksa r ON r.no_rkm_medis = p.no_rkm_medis WHERE r.no_rawat =? " ,no, tbDokter.getValueAt(tbDokter.getSelectedRow(), 0));
String NoTlp = Sequel.cariIsi(
    "SELECT p.no_tlp FROM pasien p JOIN reg_periksa r ON r.no_rkm_medis = p.no_rkm_medis WHERE r.no_rawat = ?",
    tbDokter.getValueAt(tbDokter.getSelectedRow(), 0).toString()
       ...
       // UploadImage(FileName, "media/");
//        HapusPDF();
    }//GEN-LAST:event_BtnKirimWAActionPerformed

    private void BtnWAHasilActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnWAHasilActionPerformed
        // TODO add your handling code here:
        // TODO add your handling code here:
        String FileName = "RAD_" + tbDokter.getValueAt(tbDokter.getSelectedRow(), 0).toString().replaceAll("/", "");
//       String NoTlp = Sequel.cariIsi("SELECT p.no_tlp FROM pasien p JOIN reg_periksa r ON r.no_rkm_medis = p.no_rkm_medis WHERE r.no_rawat =? " ,no, tbDokter.getValueAt(tbDokter.getSelectedRow(), 0));
String NoTlp = Sequel.cariIsi(
    ...
    tbDokter.getValueAt(tbDokter.getSelectedRow(), 0).toString()
      ...
      //  HapusPDF();
    }//GEN-LAST:event_BtnWAHasilActionPerformed

    private void BtnWAHasilKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_BtnWAHasilKeyPressed
        // TODO add your handling code here:
    }//GEN-LAST:event_BtnWAHasilKeyPressed
    ...
    private void CreatePDF(String FileName) {
        if(Kd2.getText().equals("")){
            ...
            param.put("finger2","Dikeluarkan di "+akses.getnamars()+", Kabupaten/Kota "+akses.getkabupatenrs()+"\nDitandatangani secara elektronik oleh "+tbDokter.getValueAt(tbDokter.getSelectedRow(),2).toString()+"\nID "+(finger.equals("")?kdpetugas:finger)+"\n"+Valid.SetTgl3(tbDokter.getValueAt(tbDokter.getSelectedRow(),3).toString()));  
//           Valid.MyReportPDF("rptPeriksaRadiologi.jasper","report","::[ Pemeriksaan Radiologi ]::",param);
           Valid.MyReportPDFUpload("rptPeriksaRadiologi.jasper", "report", "::[ Pemeriksaan Radiologi ]::", FileName, param);
    ...
    */
    ...
    private widget.Button BtnWAHasil;
        ...
        try {
            ...
            if(NoRawat.getText().equals("")&&kdmem.getText().equals("")&&kdptg.getText().equals("")&&TCari.getText().equals("")){
                ps=koneksi.prepareStatement(
                        ...
                        "periksa_radiologi.jam,periksa_radiologi.dokter_perujuk,periksa_radiologi.kd_dokter,penjab.png_jawab,dokter.nm_dokter,pasien.no_tlp "+
            ...
            try {
                ...
                while(rs.next()){
                    ...
                    tabMode.addRow(new Object[]{
                        ...
                        rs.getString("tgl_periksa"),rs.getString("jam"),Sequel.cariIsi("select dokter.nm_dokter from dokter where dokter.kd_dokter=?",rs.getString("dokter_perujuk")),rs.getString("nm_dokter"),rs.getString("no_tlp")
                    });
        ...
        try {
            ...
            try {
                ...
                if(rs5.next()){      
                    ...
                    NmPerujuk.setText(Sequel.cariIsi("select dokter.nm_dokter from dokter where dokter.kd_dokter=?",rs5.getString("dokter_perujuk")));
...
//                            ps.setString(3,tbDokter.getValueAt(tbDokter.getSelectedRow(),4).toString());
    ...
    try {
        ...
        try (PreparedStatement ps = koneksi.prepareStatement(
                "select gambar_radiologi.lokasi_gambar from gambar_radiologi where gambar_radiologi.no_rawat= ?")) {
            ps.setString(1, tbDokter.getValueAt(tbDokter.getSelectedRow(), 0).toString());
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    String imageUrl = "http://" + koneksiDB.HOSTHYBRIDWEB() + ":" +
                            koneksiDB.PORTWEB() + "/" + koneksiDB.HYBRIDWEB() +
                            "/radiologi/" + rs.getString("lokasi_gambar");

                    System.out.println("Mengambil gambar dari: " + imageUrl);

                    // Baca gambar dari URL
                    byte[] data;
                    try (InputStream inputStream = new URL(imageUrl).openStream()) {
                        data = org.apache.commons.io.IOUtils.toByteArray(inputStream);
                    }

                    if (data.length == 0) {
                        System.err.println("Gagal: data gambar kosong dari URL: " + imageUrl);
                        continue;
                    }

                    // Upload ke WA Gateway
                    HttpClient httpClient = new DefaultHttpClient();
                    HttpPost postRequest = new HttpPost(
                            "http://" + koneksiDB.HOSTWAWEB() + ":" + koneksiDB.PORTWEBWA() + "/" +
                    ...
                    if (response.getStatusLine().getStatusCode() == 200) {
                        JOptionPane.showMessageDialog(null,
                                "Pesan Berhasil di Antrikan di Wa Gateway!",
                                "Sukses", JOptionPane.INFORMATION_MESSAGE);
                    } else {
                        JOptionPane.showMessageDialog(null,
                                "Gagal mengupload file. Response code: " +
                    ...
                    String pesan = "==========================\n" +
                            ...
                            "hasil pemeriksaan Radiologi atas nama:\n" +
                            ...
                            "==========================\nSilakan unduh file terlampir.\n==========================\nWA Unit Radiologi: wa.me/6285182355745\nTerimakasih.";

                    String tanggaljamkirim = java.time.LocalDateTime.now()
                            .format(java.time.format.DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"));

                    String noPasien = Sequel.cariIsi(
                            "SELECT p.no_tlp FROM pasien p " +
                                    "JOIN reg_periksa r ON r.no_rkm_medis = p.no_rkm_medis " +
                                    "WHERE r.no_rawat = ?",
                            tbDokter.getValueAt(tbDokter.getSelectedRow(), 0).toString()
                    );
                    ...
                    try (PreparedStatement pswa = koneksiwa.prepareStatement(
                            "INSERT INTO wa_outbox (NOMOR, NOWA, PESAN, TANGGAL_JAM, STATUS, SOURCE, SENDER, SUCCESS, RESPONSE, REQUEST, TYPE, FILE) " +
                                    "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")) {

                        pswa.setLong(1, 0);
                        pswa.setString(2, noPasien + "@c.us");
                        pswa.setString(3, pesan);
                        pswa.setString(4, tanggaljamkirim);
                        pswa.setString(5, "ANTRIAN");
                        pswa.setString(6, "RADIOLOGI");
                        pswa.setString(7, "NODEJS");
                        pswa.setString(8, "");
                        pswa.setString(9, "");
                        pswa.setString(10, "");
                        pswa.setString(11, "FILE");
                        pswa.setString(12, FileName + ".jpg");
                        pswa.executeUpdate();
                    }
...
try {
                ...
                ps.setString(6, "RADIOLOGI");  // SOURCE
